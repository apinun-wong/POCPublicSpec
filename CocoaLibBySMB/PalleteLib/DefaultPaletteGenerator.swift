...
private let WEIGHT_SATURATION = CGFloat(0.30)
private let WEIGHT_LUMA = CGFloat(0.60)
private let WEIGHT_POPULATION = CGFloat(0.18)
...
internal class DefaultPaletteGenerator: PaletteGenerator {
    ...
    private(set) var customMutedSwatch: PaletteSwatch?
	...
	private func generateVariationColors() {
        ...
        self.customMutedSwatch = self.findColorVariation(targetLuma: 0.3, minLuma: 0.1, maxLuma: 0.5, targetSaturation: 0.6, minSaturation: 0.1, maxSaturation: 1.0)
	}
	...
	*/
	private func isAlreadySelected(swatch: PaletteSwatch) -> Bool {
		return self.vibrantSwatch == swatch || self.darkVibrantSwatch == swatch || self.lightVibrantSwatch == swatch || self.mutedSwatch == swatch || self.darkMutedSwatch == swatch || self.lightMutedSwatch == swatch || self.customMutedSwatch == swatch
	}
